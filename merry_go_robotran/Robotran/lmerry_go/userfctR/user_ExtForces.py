...
def user_ExtForces(PxF, RxF, VxF, OMxF, AxF, OMPxF, mbs_data, tsim, ixF):
    ...
    nacelle_force_ids = [
        mbs_data.extforce_id["Force_Nacelle1"],
        mbs_data.extforce_id["Force_Nacelle2"],
        mbs_data.extforce_id["Force_Nacelle3"],
        mbs_data.extforce_id["Force_Nacelle4"]
    ]

    # Si la force en cours d'évaluation est sur l'une des nacelles
    if ixF in nacelle_force_ids:
        # Force sinusoïdale alignée sur l'axe inertiel I1
        Fx = 300.0 * (1.0 - np.cos(6.0 * np.pi * tsim))
