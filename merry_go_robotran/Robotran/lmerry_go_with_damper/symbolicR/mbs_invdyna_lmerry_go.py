...
# Forward Kinematics
    ...
    ALPHA15 = ALPHA14+BETA34*s.dpt[3,9]
    ALPHA25 = C5*(ALPHA24+BETA64*s.dpt[3,9])+S5*(ALPHA34+BS94*s.dpt[3,9])
    ALPHA35 = C5*(ALPHA34+BS94*s.dpt[3,9])-S5*(ALPHA24+BETA64*s.dpt[3,9])
    ...
    BETA36 = BS36+OMp26
    BETA66 = BS66-OMp16
    ALPHA16 = ALPHA15*C6-ALPHA35*S6
    ALPHA36 = ALPHA15*S6+ALPHA35*C6
    OM17 = OM13*C7-OM33*S7
    OM27 = qd[7]+OM23
    OM37 = OM13*S7+OM33*C7
    OMp17 = C7*(OMp13-qd[7]*OM33)-S7*(OMp33+qd[7]*OM13)
    OMp27 = qdd[7]+OMp23
    OMp37 = C7*(OMp33+qd[7]*OM13)+S7*(OMp13-qd[7]*OM33)
    BS37 = OM17*OM37
    BS67 = OM27*OM37
    BS97 = -OM17*OM17-OM27*OM27
    BETA37 = BS37+OMp27
    BETA67 = BS67-OMp17
    ALPHA17 = C7*(ALPHA13+BETA23*s.dpt[2,2]+BETA33*s.dpt[3,2])-S7*(ALPHA33+BETA83*s.dpt[2,2]+BS93*s.dpt[3,2])
    ALPHA27 = ALPHA22+BETA63*s.dpt[3,2]+BS53*s.dpt[2,2]
    ALPHA37 = C7*(ALPHA33+BETA83*s.dpt[2,2]+BS93*s.dpt[3,2])+S7*(ALPHA13+BETA23*s.dpt[2,2]+BETA33*s.dpt[3,2])
    OM18 = qd[8]+OM17
    ...
    ALPHA110 = C10*(ALPHA13+BS13*s.dpt[1,3])-S10*(ALPHA33+BETA73*s.dpt[1,3])
    ALPHA210 = ALPHA22+BETA43*s.dpt[1,3]
    ALPHA310 = C10*(ALPHA33+BETA73*s.dpt[1,3])+S10*(ALPHA13+BS13*s.dpt[1,3])
    OM111 = qd[11]+OM110
    ...
    ALPHA111 = ALPHA110+BETA310*s.dpt[3,15]
    ALPHA211 = C11*(ALPHA210+BETA610*s.dpt[3,15])+S11*(ALPHA310+BS910*s.dpt[3,15])
    ALPHA311 = C11*(ALPHA310+BS910*s.dpt[3,15])-S11*(ALPHA210+BETA610*s.dpt[3,15])
    ...
    OM113 = OM13*C13-OM33*S13
    OM213 = qd[13]+OM23
    OM313 = OM13*S13+OM33*C13
    OMp113 = C13*(OMp13-qd[13]*OM33)-S13*(OMp33+qd[13]*OM13)
    OMp213 = qdd[13]+OMp23
    OMp313 = C13*(OMp33+qd[13]*OM13)+S13*(OMp13-qd[13]*OM33)
    BS313 = OM113*OM313
    BS613 = OM213*OM313
    BS913 = -OM113*OM113-OM213*OM213
    BETA313 = BS313+OMp213
    BETA613 = BS613-OMp113
    ALPHA113 = C13*(ALPHA13+BETA23*s.dpt[2,4]+BETA33*s.dpt[3,4])-S13*(ALPHA33+BETA83*s.dpt[2,4]+BS93*s.dpt[3,4])
    ALPHA213 = ALPHA22+BETA63*s.dpt[3,4]+BS53*s.dpt[2,4]
    ALPHA313 = C13*(ALPHA33+BETA83*s.dpt[2,4]+BS93*s.dpt[3,4])+S13*(ALPHA13+BETA23*s.dpt[2,4]+BETA33*s.dpt[3,4])
    OM114 = qd[14]+OM113
...
# Backward Dynamics
    ...
    Cq110 = -s.trq[1,10]+Cq111+s.In[1,10]*OMp110-s.In[5,10]*OM210*OM310+s.In[9,10]*OM210*OM310-Fs210*s.l[3,10]- \
 	  s.dpt[3,15]*(-Fq311*S11+Fs212*C11)
    Cq210 = -s.trq[2,10]+s.In[1,10]*OM110*OM310+s.In[5,10]*OMp210-s.In[9,10]*OM110*OM310+Cq212*C11-Cq311*S11+Fq111* \
 	  s.dpt[3,15]+Fs110*s.l[3,10]
    Cq310 = -s.trq[3,10]-s.In[1,10]*OM110*OM210+s.In[5,10]*OM110*OM210+s.In[9,10]*OMp310+Cq212*S11+Cq311*C11
    ...
    Fq15 = Fs16*C6+Fs36*S6
    Fq35 = -Fs16*S6+Fs36*C6
    ...
    Fq34 = Fs34+Fq35*C5+Fs26*S5
    Cq14 = -s.trq[1,4]+Cq15+s.In[1,4]*OMp14-s.In[5,4]*OM24*OM34+s.In[9,4]*OM24*OM34-Fs24*s.l[3,4]-s.dpt[3,9]*(-Fq35* \
 	  S5+Fs26*C5)
    Cq24 = -s.trq[2,4]+s.In[1,4]*OM14*OM34+s.In[5,4]*OMp24-s.In[9,4]*OM14*OM34+Cq26*C5-Cq35*S5+Fq15*s.dpt[3,9]+Fs14* \
 	  s.l[3,4]
    Cq34 = -s.trq[3,4]-s.In[1,4]*OM14*OM24+s.In[5,4]*OM14*OM24+s.In[9,4]*OMp34+Cq26*S5+Cq35*C5
    Fs13 = -s.frc[1,3]+s.m[3]*(ALPHA13+BETA33*s.l[3,3])
    Fs23 = -s.frc[2,3]+s.m[3]*(ALPHA22+BETA63*s.l[3,3])
    Cq13 = -s.trq[1,3]+s.In[1,3]*OMp13-s.In[5,3]*OM23*OM33+s.In[9,3]*OM23*OM33+Cq110*C10+Cq113*C13+Cq14*C4+Cq17*C7+ \
